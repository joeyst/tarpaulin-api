...
function updateModelsByFilter(collectionName, filterOptions, getObj) {
  return async (req, _, next) => {
    filterOptions = mapValues(filterOptions, value => get(req, value))
    if (filterOptions._id) { filterOptions._id = new ObjectId(filterOptions._id) }
    await getMongoCollection(collectionName).update(filterOptions, { $set: req[getObj] }) 
    next()
  }
}

function sendStatusCodeWithAttribute(statusCode, getAttr, key=null) {
