...
function sendStatusCodeWithAttribute(statusCode, attr, withKey=true) {
  return async (req, res) => {
    if (withKey) {
      res.status(statusCode).send({ [last(attr.split("."))]: get(req, attr, null) })
    } else {
      res.status(statusCode).send(get(req, attr, null))
    }
  }
}
